...
pub fn register_number_prototype(proto: &Rc<RefCell<dyn JSObject>>) {
    ...
    set_prototype_method(proto, "toString", |this, _args| number_to_string_proto(this));
    set_prototype_method(proto, "toFixed", |this, args| number_to_fixed(this, args));
    set_prototype_method(proto, "toExponential", |this, args| number_to_exponential(this, args));
    set_prototype_method(proto, "toPrecision", |this, args| number_to_precision(this, args));
}
...
pub fn number_to_fixed(obj: &Value, args: &[Value]) -> Result<Value, RuntimeError> {
    let n = obj.to_number();
    if n.is_nan() { return Ok(Value::string("NaN")); }
    if n.is_infinite() {
        return if n.is_sign_positive() { Ok(Value::string("Infinity")) } else { Ok(Value::string("-Infinity")) };
    }
    let digits = if args.is_empty() { 0 } else { args[0].to_number() as usize };
