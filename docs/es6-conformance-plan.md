# ES6 一致性计划（M2 收尾 → M6）—— 已冻结为历史

> **⚠️ 当前有效的计划是 `docs/es6-conformance-phase2.md`（第二阶段计划书，M5 → M8，2026-09-25）。**
> 本文件覆盖 M0 → M4 的工作日志与历史基线，其 §3.1/§3.3/§4 的计划部分已随进展过时（详见 §1.3）；**排期与验收请以第二阶段计划书为准**。
...
| 类（构造 / 方法 / 静态 / 访问器 / extends / super） | 在 | ⚠️ | M2 已交付，M2' 补齐计算属性名与 `new.target`；剩余是求值顺序与 own-property 长尾（阶段二计划书 §5 M7-P2，705 条） |
| 模板字符串（含 tagged 基础形态） | 在 | ⚠️ | 插值已支持；`raw` 冻结数组与 call-site 缓存未做 |
| 解构（数组 / 对象 / 参数 / 赋值目标 / 嵌套 / 默认值 / rest） | 在 | ✅ | M1 + M1 收尾 |
| 默认参数、剩余参数、展开（调用 / `new` / 数组 / 对象） | 在 | ✅ | |
| `for-of` / `for-in`、迭代协议 | 在 | ⚠️ | 双路径迭代已可用，`IteratorClose`/`GetIterator` 已入协议（§2.1ab/§2.1ac）；剩余 321 条 + **spread 尚未走协议**（阶段二计划书 §5 M7-P3） |
| **生成器 `function*` / `yield`** | 在 | ✅ | M4-G1/G2/G3 与 §2.1u–§2.1x 已交付；`statements/generators` 65.5%、`expressions/generators` 64.7%，达标（§4 M4）；剩余 203 条见本文件 §2.3 |
...
| `JSON` / `Date` | 在（ES5 但属必备） | ⚠️ | `JSON` 已交付（112 通过 / 2 失败，§2.1m）；**`Date` 594 例 0 开工且不在度量内 —— 已交给阶段二计划书 §5 M8-V2 出结论，不允许继续悬空** |
...
### 1.3 第二阶段的计划已另立文档（2026-09-25）

**M5 → M8 的计划不在本文件里**，见 **`docs/es6-conformance-phase2.md`（第二阶段计划书）**。本文件自此作为 **M0 → M4 的工作日志与历史基线**冻结，不再追加计划性内容。

**为什么另立**：本文件是「工作日志 + 计划」的混合体，逐批追加后已经 1400 余行，而其中的计划部分（§3.1/§3.3/§4）随每一批就过时一次，读者无法判断哪一版是当前的。第二阶段的计划书要求自包含、可排期、可验收，因此重开一份。

**本次对旧文档做的订正**（只改与事实不符之处，不追加计划）：

1. **§1.2 的"现状"列**有六行与事实不符，已订正：生成器仍标 ❌（实际 `statements/generators` 65.5%、`expressions/generators` 64.7%）、计算属性名与 `new.target` 仍标 ❌（M2' 已交付）、`for-of` 一行未反映 §2.1ab/§2.1ac 的三条协议步骤、`JSON`/`Date` 未拆开、内置对象的通过率未给出。
2. **§1.2 的 `Date` 行**：原判"在（ES5 但属必备）"，实际 594 例 0 开工且从未进入度量。已标注为**悬空项**，交给第二阶段计划书的 M8-V2 出结论。
3. **§2.1a 增加"runner 覆盖面"一行**：25586 / 53568 = 47%。未覆盖的部分里有约 4920 条属于本文件 §1.2 承诺过的 M5/M6 —— 这是"计划承诺面"与"度量面"不一致的直接证据。
4. **§3.3 与 §4 加历史横幅**：两节的排序基于 §2.1y 之后的失败池，未包含从未进入度量的 M5/M6，已指向第二阶段计划书。
5. **§4-G4 补注**：`for-of`/解构/展开的联调只部分交付（展开仍走 `array_like_elements` 快照，未走迭代协议）。
6. **§7 新增 7.1「已发生的风险」**：跳过表错配（+4218）、runner 不补 `onlyStrict`（+523）、引擎崩溃打断整轮回归，以及度量面与承诺面不一致。四者都是**度量/工具层**的风险，不是引擎语义风险，但单次影响都超过任何一个语义批次，原计划没有登记。
...
| 全量耗时基线 | 2m05s |
| runner 覆盖面 | 25586 / 53568 个测试文件（47%）——**未覆盖的 27982 里约 4920 条属本文件 §1.2 承诺过的 M5/M6**，清单见阶段二计划书 §2.3 |
...
> **2026-09-25**：本节已被**阶段二计划书**取代（原排序只覆盖失败池，漏掉了从未进入度量、也从未开工的 M5/M6 共约 4920 条）。下面保留作历史记录。
...
> 后续里程碑（M5 集合与反射、M6 TypedArray、M7 协议与长尾一致性、M8 度量与文档收口）见 **`docs/es6-conformance-phase2.md`** —— 其中 M5/M6 至今 0 开工，其套件也不在 runner 的 `SUITES` 里，因此完全不在当前 KPI 之内。
...
| 度量面与承诺面不一致 | M5/M6 的 11 个套件不在 `SUITES` 里 | 约 4920 条计划内测试从未进入 KPI，里程碑「进度」不可见 | **已实测清楚**：单纯入册只多出 1552 条纯失败、通过数零增长（通过率 75.59% → 69.43%）。结论是必须**入册与解除特性门控联动** —— 规则见阶段二计划书 §2.2 |
