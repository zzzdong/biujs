...
| 类（构造 / 方法 / 静态 / 访问器 / extends / super） | 在 | ⚠️ | M2 已交付，M2' 补齐计算属性名与 `new.target`；剩余是求值顺序与 own-property 长尾（§1.3.4 M7-P2，705 条） |
| 模板字符串（含 tagged 基础形态） | 在 | ⚠️ | 插值已支持；`raw` 冻结数组与 call-site 缓存未做 |
| 解构（数组 / 对象 / 参数 / 赋值目标 / 嵌套 / 默认值 / rest） | 在 | ✅ | M1 + M1 收尾 |
| 默认参数、剩余参数、展开（调用 / `new` / 数组 / 对象） | 在 | ✅ | |
| `for-of` / `for-in`、迭代协议 | 在 | ⚠️ | 双路径迭代已可用，`IteratorClose`/`GetIterator` 已入协议（§2.1ab/§2.1ac）；剩余 321 条 + **spread 尚未走协议**（§1.3.4 M7-P3） |
| **生成器 `function*` / `yield`** | 在 | ✅ | M4-G1/G2/G3 与 §2.1u–§2.1x 已交付；`statements/generators` 65.5%、`expressions/generators` 64.7%，达标（§4 M4）；剩余 203 条见 §2.3 |
| `Symbol` 与 well-known symbols | 在 | ⚠️ | `toStringTag` / `toPrimitive` / `hasInstance` / `species` / `isConcatSpreadable` 已接入；`built-ins/Symbol` 37 通过 / 34 失败 |
...
| 内置对象 ES6 增补（Object/Array/String/Number/Math 新方法） | 在 | ⚠️ | 通过率：Object 83.9%、Array 69.4%、String 60.4%、Number 62.7%、Math 93.1%、JSON 98.2%；残余为长尾（§3.1） |
| `JSON` / `Date` | 在（ES5 但属必备） | ⚠️ | `JSON` 已交付（112 通过 / 2 失败，§2.1m）；**`Date` 594 例 0 开工且不在度量内 —— 必须由 §1.3.4 M8-V2 给出实现或降级的明确结论** |
| 计算属性名、`**`（ES2016）、`new.target` | 在 | ✅ | M2' 已交付（§2.1b） |
...
**结论：三条硬约束与验证方法论全部有效，里程碑骨架也仍然成立；但"剩余工作的定义"和"度量口径"已经和原计划脱节，需要重订。**

#### 1.3.1 与原计划相符的部分

| 项 | 判定 |
|----|------|
| §1.1 G1 strict-only | ✅ 而且被**加强了**：§2.1aa 接上 oxc 语义构建器后，strict 早期错误（重复形参、保留字作标识符、标签函数声明）首次真正生效 |
| §1.1 G2/G3（ES6 特性集、无 eval/with） | ✅ 无越界；跳过表里 `eval(` / `new Function(` / `with (` 仍是拒绝项 |
| §4 的 M2' → M3 → M4 顺序 | ✅ 按序执行，无跳步 |
| §4 各里程碑的"完成标准" | ✅ **M2'、M3、M4 三者的验收条件都已达标**：M2' 三项套件 ≥ 50%（75% / 90% / 89%）；M3 的 Object/Array/String 通过率 ≥ 50%（83.9% / 69.4% / 60.4%）；M4 的 generators ≥ 40%（65.5% / 64.7%） |
| §6 的三级验证 + 内存护栏 | ✅ 每次提交都跑；内存上限是必须的，中间确实靠它拦下过一次无界物化 |
| §6.2 跳过表治理 | ✅ 已收敛（§2.1y）：表里不再有"因为当初没实现而随手加"的条目 |
| §7 的风险方向（挂起/恢复是最大改动、泛型路径成本） | ✅ 判断正确：M4 确实是最贵的一批；Array 泛型长尾确实是最黏的一批 |

#### 1.3.2 与原计划不符的部分（按严重度）

1. **度量口径变了，而且没有定义过"计划内测试面"。** §2.1 的基线是 `10240 执行 / 3908 通过`，现在同一个 runner 跑出 `17477 执行 / 13211 通过`——不是引擎涨了 3 倍，是 §2.1y 修掉两条错误的跳过规则后分母变大了。计划从头到尾没有写清"通过率是相对哪个集合算的"，所以 §2.1/§3.1/§4 里的每一个百分比都要重新解释。

2. **M5/M6 从未开工，而且从未被度量。** §1.2 承诺 `Map`/`Set`/`WeakMap`/`WeakSet`/`Promise`/`Proxy`/`Reflect`/`TypedArray`/`ArrayBuffer`/`DataView` 都在范围内，§4 也给它们排了 M5/M6——但这些套件**根本不在 `runner::SUITES` 名单里**，既不计通过也不计跳过。它们完全在 KPI 之外：

   | 未纳入度量的套件 | 文件数 |
   |------|------|
   | `built-ins/TypedArray` | 1446 |
   | `built-ins/TypedArrayConstructors` | 738 |
   | `built-ins/Promise` | 677 |
   | `built-ins/DataView` | 561 |
   | `built-ins/Set` | 383 |
   | `built-ins/Proxy` | 311 |
   | `built-ins/ArrayBuffer` | 221 |
   | `built-ins/Map` | 204 |
   | `built-ins/Reflect` | 153 |
   | `built-ins/WeakMap` / `WeakSet` | 141 / 85 |
   | 合计 | **≈ 4920** |

   runner 目前只覆盖 test262 的 **25586 / 53568 = 47%**。也就是说"75.59%"这个数字描述的是**不到一半的测试面**，而缺的那一半里约 4920 条正是计划承诺要做的东西。

3. **M3-B6 的 `Date` 没做。** §1.2 把它写成"在（ES5 但属必备）"，§4-B6 只交付了 JSON，`Date` 594 例至今 0 开工、也不在度量内（`Date` 只作为源码模式出现在跳过表里）。这是一个**没有闭环的承诺**：要么实现，要么明确降级并写进 §1.2 的范围外栏。

4. **§1.2 的"现状"列大面积过时。** 生成器仍标 ❌（实际两个目录 65%）、计算属性名/`new.target` 仍标 ❌（M2' 已交付）、类的 `super` 未提及。

5. **出现了没有任何里程碑覆盖的新工作类别。** §2.1ab / §2.1ac 做掉的是**协议步骤**（`RequireObjectCoercible`、`IteratorClose`、`GetIterator`），它们横切 class/for-of/解构/参数绑定，收益一次落在 12 个套件上——原计划的 B1-B6（按内置对象分）与 G1-G4（按生成器分）都装不下这类工作。同样地，§2.1aa 的解析期校验层也不属于任何里程碑。

6. **失败池的成分与计划假设不同。** §3.1 假设残余是"缺方法/语义细节"；实际是**长尾语义**，且最大的两块是 `class`（705）与 `Array` 回调方法的**泛型/访问器路径**（≈450），都不是"补一个方法"能解决的。另外池里混着 644 条**非 ES6 驱动**的失败（`class-fields-public` 838 条中的大部分、`resizable-arraybuffer`、`change-array-by-copy`…），真实范围内失败是 **3622**。

7. **§7 风险登记没有覆盖真正发生过的风险**：跳过表错配（§2.1y，一次 +4218）、runner 不补 `onlyStrict`（§2.1aa，一次 +523）、引擎崩溃打断整轮回归（§2.1ac，Rust 栈溢出，进程 SIGABRT 且报告看不出元凶）。三者都是**度量/工具层**的风险，不是引擎语义风险。

#### 1.3.3 修订后的度量口径（先定这个，其余才有意义）

**计划内测试面** = `runner::SUITES` 列出的套件 ∪ `§1.2 判定为"在范围"的全部 built-ins 目录`。报告里并列三个数：

| 数字 | 含义 |
|------|------|
| **计划内通过数 / 计划内执行数** | 主指标（绝对数优先，§6.1 的门槛也用它） |
| 目标套件通过率 | 质量指标，用于判断"这批做完没有" |
| 范围外排除数 | 必须能逐条说出理由（§6.2） |

**P0 动作**：把 M5/M6 的 11 个套件加进 `SUITES`。预期效果：执行数 17477 → ≈ 22400，通过数略微上升（个别用例零散通过），整体通过率从 75.59% 降到约 **60% 上下** —— 这是**更真实的 ES6 覆盖度**，也是 M5/M6 唯一可行的进度可见化方式。绝对通过数不会下降，符合 §6.1。

#### 1.3.4 修订后的里程碑

原 `§4` 的 M2' / M3 / M4 已达成各自验收条件，标记为已完成；剩余工作重排为四段。

**M5 集合与反射（ES6 承诺内，当前 0%）** —— 先 P0 入册，再按依赖顺序做：

| 任务 | 内容 | 规模（文件） | 依赖 |
|------|------|--------------|------|
| C1 `Map` / `Set` | 插入序、`size`、`forEach`、`keys`/`values`/`entries`、`Symbol.iterator`；复用既有迭代器基建 | 204 / 383 | 无（迭代器基建已就绪） |
| C2 `WeakMap` / `WeakSet` | `Rc` 场景下弱引用退化为强引用，偏差记入 §2.3 | 141 / 85 | 无 |
| C3 `Promise` | 状态机 + 微任务队列（VM 主循环需要一个明确的 draining 点） | 677 | 需要调度点，**成本最高，放最后** |
| C4 `Proxy` / `Reflect` | 陷阱分发；`Reflect` 静态方法 | 311 / 153 | 依赖 `[[DefineOwnProperty]]`/`[[Get]]`/`[[Set]]` 已成型（M3 已交付） |

**M6 TypedArray / ArrayBuffer / DataView（ES6 承诺内，当前 0%）**：

| 任务 | 内容 | 规模 |
|------|------|------|
| T1 `ArrayBuffer` + `DataView` | 共享 buffer、偏移、越界 | 221 + 561 |
| T2 TypedArray | 11 种视图 + `TypedArrayConstructors` 的构造语义 | 1446 + 738 |
| T3 detach / resizable | `resizable-arraybuffer` 相关（当前在失败池里以"缺 `Uint8Array`"形式出现 88 条） | — |

**M7 协议与长尾一致性（当前失败池主干 3622 条）**：

| 任务 | 内容 | 规模 | 来源 |
|------|------|------|------|
| P1 Array 回调方法的泛型/访问器路径 | `reduceRight` 79、`lastIndexOf` 60、`reduce` 54、`indexOf` 50、`map`/`some`/`every`/`filter`/`forEach` 各 ≈40、`sort` 33 —— 根因是 builtin 层读不到原型链上的访问器 | ≈ 450 | §2.3 已登记 |
| P2 class 求值顺序与 own-property 语义 | `Expected SameValue` 345、`X should be an own property` 79、`Cannot read undefined` 66 | 705 | §2.1ac 新暴露 |
| P3 迭代协议剩余 | `for-of` 321：`IteratorClose` 计数、`iterator-next-result-type`、**spread 走 `GetIterator`** | 321 | §2.3 新登记 |
| P4 Object 描述符长尾 | `defineProperty` 131 + `defineProperties` 86 | 217 | — |
| P5 数据模型三债 | 脚本完成值不可靠、闭包捕获对象快照点错误、`ArrayPushSpread` | — | §2.3 新登记 |
| P6 生成器剩余 | `statements/generators` 87 + `expressions/generators` 96 + `yield` 20 | 203 | §2.3 已逐条登记 |

**M8 度量与文档收口**：

| 任务 | 内容 |
|------|------|
| V1 覆盖率 | `SUITES` 47% → 明确"计划内测试面"，把不在范围的目录在表内写明理由 |
| V2 `Date` 决策 | 实现（594）或降级进 §1.2 范围外栏 —— **不允许继续悬空** |
| V3 RegExp 驱动失败的归类 | `String/prototype` 的 `replace`/`match`/`search`/`split` 合计 145 条、`Array` 侧若干，在 RegExp 缺席时不可能通过；跳过表用的是 `RegExp` 子串，覆盖不全，需在 §1.2/§6.2 里明确"这部分不计入 M7 的 KPI" |
| V4 文档一致性 | §1.2 现状栏、`es6-feature-support.md`、README 三者对齐 |

#### 1.3.5 建议的开工顺序

1. **P0**：把 M5/M6 套件加进 `SUITES`（纯度量改动，让后面每一步可见）——**先做这个**。
2. **M7-P3 的 spread 走协议**：独立、边界清楚、刚暴露，且与 P1 同源。
3. **M7-P1 Array 泛型/访问器**：单批最大（≈450），但需要把 builtin 层的索引读写上移到 VM，是跨层改动，留足时间。
4. **M5-C1/C2（`Map`/`Set`/`WeakMap`/`WeakSet`）**：ES6 承诺内最早能整体交付的板块，且不依赖新机制。
5. **M8-V2 `Date` 决策**：一件小事，但拖着会让 §1.2 一直说谎。
6. **M5-C4 → M6 → M5-C3**：`Proxy`/`Reflect` 依赖已就绪，`Promise` 需要先定微任务调度点。
...
| 全量耗时基线 | 2m05s |
| runner 覆盖面 | 25586 / 53568 个测试文件（47%）——**未覆盖的 27982 里约 4920 条属计划承诺的 M5/M6**，见 §1.3.2 |
...
> **2026-09-25 二次修订**：本节被 §1.3.4 取代（原排序只覆盖失败池，漏掉了从未进入度量、也从未开工的 M5/M6 共约 4920 条）。下面保留作历史记录。
...
> **状态（2026-09-25）**：M2' / M3 / M4 的验收条件**均已达标**（§1.3.1），本节保留作历史记录。
> 后续里程碑（M5 集合与反射、M6 TypedArray、M7 协议与长尾一致性、M8 度量与文档收口）见 **§1.3.4** —— 其中 M5/M6 至今 0 开工，且其套件尚未进入 runner 的 `SUITES`，因此完全不在当前 KPI 之内。
...
| G4 与 `for-of`、解构、展开的联调 | 部分交付（§2.1ab/§2.1ac 把 `RequireObjectCoercible` / `IteratorClose` / `GetIterator` 三条协议步骤补上）；**spread 仍走 `array_like_elements` 快照，未走协议** → 转入 M7-P3 |
...
### 7.1 已发生的风险（2026-09-25 补记）

前三类都是**度量/工具层**的，不是引擎语义风险 —— 计划最初没有把它们列进来，但它们单次的收益都比任何一个语义批次大（+4218 / +523 / 阻断整轮回归），因此必须进登记表。

| 风险 | 实际发生 | 影响 | 现有对策 |
|------|----------|------|----------|
| 跳过表错配 | `Flag::Generated` 与 `$DONOTEVALUATE` 被当成特性门控（§2.1y） | 6652 个可执行测试被排除在分母外，度量严重低估 | 已修；并确立「跳过项必须能逐条说出理由」（§6.2） |
| runner 与 test262 约定不符 | `onlyStrict` 用例没有补指令序言里的 `use strict`（§2.1aa） | 513 条早期错误用例按 sloppy 解析，全部误判 | 已修；**待办**：既无 `onlyStrict` 也无 `noStrict` 的用例按规范要跑 sloppy + strict 两遍 |
| 引擎崩溃打断整轮回归 | 字符串包装对象迭代无限递归（§2.1ac） | 进程 SIGABRT，报告里看不出是哪个用例 | 已修；新增 `BIUJS_TEST262_TRACE=1` 逐条打印测试路径 |
| 度量面与承诺面不一致 | M5/M6 的 11 个套件不在 `SUITES` 里 | 约 4920 条计划内测试从未进入 KPI，里程碑「进度」不可见 | **未修** → §1.3.3 的 P0 |
