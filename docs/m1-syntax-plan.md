...
> **状态**：**T1-T8 已完成**（提交：`8abdb9e` 规划，`92897b3` 起实现，T1-T4 与 T5-T8 两次提交）
> **前置**：跨块活跃性分析修复（`92897b3`）已完成，寄存器分组分配在正确 liveness 下工作
> **基线**：test262 3608/8485（42.52%），另有 15287 个测试因特性缺失被跳过
> **当前**：3873/10217（37.91%）—— 绝对通过数 3608 → **3873**（+265），
> 另解锁约 1700 个此前被跳过的测试（更多测试进入执行队列，故通过率分母变大）
>
> 实现结果摘要：
> - T1 双路径迭代协议（数组/字符串原生快路径 + `Symbol.iterator` 协议慢路径）
> - T2 `for-of` / `for-in`（后者复用前者 + `Object.keys`）
> - T3 模板字符串插值（新增 `ToString` 指令，避免 `Addx` 的算术语义）
> - T4 默认参数（函数序言 `undefined` 检查，可引用前置参数）
> - T5 剩余参数（`MakeRest`，复用每帧实参计数）
> - T6 展开（调用 / `new` / 数组字面量；`CallSpread`/`NewSpread` + `construct` 助手）
> - T7/T8 数组与对象解构（声明 / 参数 / catch / for-of 头部；默认值、rest、嵌套）
>
> **未完成**（转入 M1 后续或 M4）：对象展开 `{...x}`、`new` 的展开在部分
> 构造器上的语义细节、tagged template 的 `raw`/冻结数组、生成器（for-of 的
> 大量用例依赖 `function*`），以及 `let` 循环变量按轮次捕获。
